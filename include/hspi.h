...
static inline void hspi_prepare_txrx(uint32_t bytecount_tx, uint32_t bytecount_rx)
{
	uint32_t bitcount_tx = bytecount_tx * 8 - 1;
	uint32_t bitcount_rx = bytecount_rx * 8 - 1;
	WRITE_PERI_REG(SPI_FLASH_USER1(HSPI), ((bitcount_tx & SPI_USR_OUT_BITLEN) << SPI_USR_OUT_BITLEN_S)  | ((bitcount_rx & SPI_USR_DIN_BITLEN) << SPI_USR_DIN_BITLEN_S) );
}
