...
void hspi_init(void)
{
	...
	uint32_t regvalue = SPI_FLASH_DOUT | SPI_FLASH_USR_DIN;
	regvalue &= ~(BIT2 | SPI_FLASH_USR_ADDR | SPI_FLASH_USR_DUMMY | SPI_USR_COMMAND | SPI_DOUTDIN); //clear bit 2 see example IoT_Demo
...
void hspi_send_data_uint16(const uint16_t *data, int32_t count)
{
	uint32_t d, i, k, words_to_transfer;

	k = 0;
	count = (count + 1)/2; /* Convert to 32-bit word count */
	while (count > 0) {
		words_to_transfer = __min(count , SPIFIFOSIZE);
		hspi_wait_ready();
		hspi_prepare_tx( words_to_transfer<<2 ); /* Transfer size in bytes - conwert 32bit words to bytes */
		for (i = 0; i < words_to_transfer; ++i) {
			spi_fifo[i] = pgm_read_dword(&data[k]);
			k+=2;
		}
		hspi_start_tx();
		count -= words_to_transfer;
	}
}
//===========================================================================================
